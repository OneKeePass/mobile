...
class OkpDbService: NSObject {
  ...
  @objc
  func registerPasskeyIdentities(_ resolve: @escaping RCTPromiseResolveBlock,
                                  reject _: @escaping RCTPromiseRejectBlock) {
    ...
    DispatchQueue.global(qos: .userInteractive).async {
      let resultJson = DbServiceAPI.iosAppGroupSupportService().invoke("passkey_get_all", "{}")
      self.logger.debug("registerPasskeyIdentities: passkey_get_all returned \(resultJson)")

      guard
        let data  = resultJson.data(using: .utf8),
        let dict  = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
        let items = dict["ok"] as? [[String: Any]]
      else {
        resolve("{}")
        return
      }

      let identities: [ASPasskeyCredentialIdentity] = items.compactMap { item in
        self.logger.debug("Item is \(item)")
        self.logger.debug("Item keys are \(Array(item.keys))")
        self.logger.debug("Item values are \(Array(item.values))")
        
        var rpId     = item["rp_id"]                as? String
        var userName = item["username"]              as? String
        var credB64  = item["credential_id_b64url"]  as? String
        var uhB64    = item["user_handle_b64url"]    as? String
        var credData = Data(base64Encoded: credB64!)
        var uhData   = Data(base64Encoded: uhB64!)
        
        self.logger.debug("rpId \(rpId), userName \(userName),  credB64  \(credB64!),  uhB64 \(uhB64!),  credData \(credData),  uhData \(uhData)")
           
        guard
          let rpId     = item["rp_id"]                as? String,
          let userName = item["username"]              as? String,
          let credB64  = item["credential_id_b64url"]  as? String,
          let uhB64    = item["user_handle_b64url"]    as? String,
          let credData = self.decodeBase64URL(credB64),
          let uhData   = self.decodeBase64URL(uhB64)
        else { return nil }
        return ASPasskeyCredentialIdentity(
          ...
          userHandle:             uhData,
          recordIdentifier:       item["entry_uuid"] as? String)
      }

      guard !identities.isEmpty else {
        self.logger.debug("registerPasskeyIdentities: no passkeys found")
        resolve("{}")
        return
      }

      self.logger.debug("registerPasskeyIdentities: saving \(identities.count) identity(ies)")
      ASCredentialIdentityStore.shared.saveCredentialIdentities(identities) { success, error in
        if let error = error {
          self.logger.error("registerPasskeyIdentities failed: \(error)")
        } else {
          self.logger.debug("registerPasskeyIdentities: saved, success=\(success)")
        }
      }
      resolve("{}")
    }
