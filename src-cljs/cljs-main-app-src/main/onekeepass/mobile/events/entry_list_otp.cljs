(ns onekeepass.mobile.events.entry-list-otp
  "Current tokens for the rows of an entry list.
   ...
   icon data-urls do. Being per database means closing one leaves another's codes alone,
   and that the paths which already drop a database's state - the dissoc in
   ':close-kdbx-completed', the cleared-db reduce in the android autofill init - take the
   codes with them rather than having to be told about them separately."
  (:require [onekeepass.mobile.background :as bg]
            [onekeepass.mobile.events.common :refer [active-db-key assoc-in-key-db
                                                     assoc-in-selected-db get-in-key-db
                                                     on-ok]]
...
(reg-event-fx
 ...
 (fn [{:keys [db]} [_event-id]]
   (let [uuids (vec (keys (get-in-key-db db [:entry-list-otp :tokens])))]
     (if (seq uuids)
       {:fx [[:entry-list-otp/bg-current-otps [(active-db-key db) uuids]]]}
       {}))))

;; Drops the codes of one database. Closing one needs no such call - its whole key-db goes
;; with it - but locking leaves the key-db in place, and a locked database must not leave
;; codes behind
(reg-event-fx
 :entry-list-otp/clear-db
 (fn [{:keys [db]} [_event-id db-key]]
   (cancel-timer batch-timer)
   (cancel-timer refresh-timer)
   (reset! pending #{})
   (reset! in-flight #{})
   {:db (assoc-in-selected-db db db-key [:entry-list-otp] nil)}))
